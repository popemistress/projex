...
import { CloseBoardConfirmation } from "./components/CloseBoardConfirmation";
...
export default function BoardPage({ isTemplate }: { isTemplate?: boolean }) {
  ...
  const renderModalContent = () => {
    return (
      <>
        <Modal
          modalSize="sm"
          isVisible={isOpen && modalContentType === "CLOSE_BOARD"}
        >
          <CloseBoardConfirmation boardPublicId={boardId ?? ""} />
        </Modal>
  ...
  return (
    <>
      <PageHead
        title={`${boardData?.name ?? (isTemplate ? "Board" : "Template")} | ${workspace.name ?? "Workspace"}`}
      />

  ...
  getAuthorizationUrl: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (input.provider === "trello") {
        const appName = encodeURIComponent("Self Maxing (Projex)");
        const url = `${urls[input.provider]}/authorize?key=${apiKey}&name=${appName}&expiration=never&response_type=token&scope=read&return_url=${env("NEXT_PUBLIC_BASE_URL")}/settings/trello/authorize&callback_method=fragment`;
        return { url };
      }

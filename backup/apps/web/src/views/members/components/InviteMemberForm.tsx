...
export function InviteMemberForm({
  ...
  let isYearly = false;
  let price = "$10/month";
  let billingType = "monthly billing";
  ...
  if (teamSubscription?.periodStart && teamSubscription.periodEnd) {
    ...
    price = isYearly ? "$8/month" : "$10/month";
    billingType = isYearly ? "billed annually" : "billed monthly";
  }
            ...
            {hasTeamSubscription || hasProSubscription ? (
              <div>
                <span className="font-medium text-emerald-500 dark:text-emerald-400">
                  {hasTeamSubscription ? "Team Plan" : "Pro Plan ∞"}
                </span>
                <p className="mt-1">
                  {unlimitedSeats
                    ? "You have unlimited seats with your Pro Plan. There is no additional charge for new members!"
                    : "Adding a new member will cost an additional ${price} (${billingType}) per seat."}
                </p>
              </div>
            ) : (
              <div>
                ...
                <p className="mt-1">
                  {"Inviting members requires a Team Plan. You'll be redirected to upgrade your workspace."}
                </p>
              </div>
            )}
          </div>
